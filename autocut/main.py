...
def main():
    ...
    if args.transcribe:
        ...
        Cutter(args).run() # 合并视频
    elif args.daemon:
        from .daemon import Daemon

        Daemon(args).run() # 遍历文件夹，对其中的视频文件进行剪辑
    elif args.s:
        utils.compact_rst(args.inputs[0], args.encoding) # 将srt转化为md格式
    else:
        logging.warning("No action, use -c, -t or -d") # 没有指定操作 请使用 -c -t 或 -d
